...
const Login = () => {
  ...
  const register = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/api/register', formRegister, {
      });
  ...
  const login = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:8000/api/login', formLogin, {
      ...
      <div className="box w-full max-w-5xl bg-white rounded-2xl shadow-elevated overflow-hidden">
        <div className="inner-box flex flex-col md:flex-row h-[600px]">
          ...
          <div className="forms-wrap md:w-1/2 p-8">
            {/* Sign In Form */}
            <form onSubmit={login} autoComplete="off" className={isSignUp ? 'hidden' : 'block'}>
            ...
            {/* Sign Up Form */}
            <form onSubmit={register} autoComplete="off" className={isSignUp ? 'block' : 'hidden'}>
              <div className="logo mb-6">
                <h4 className="text-2xl font-bold">Trans <span className="text-transco-gold">CO</span></h4>
              </div>
              ...
              <input
                  ...
                  required
                  minLength={4}
                  placeholder=" " // keeps floating label style
                />
                <label
                  htmlFor="name"
                ...
                <input
                  ...
                  className="w-full input-field px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-transco-gold transition-all"
                  required
                  placeholder=" " // needed to trigger the floating label
                />
                <label
                  htmlFor="email"
                  className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none transition-all duration-300"
